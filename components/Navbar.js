...
const Navbar = ({ cart, addToCart, removeFromCart, clearCart, subTotal }) => {
    ...
    console.log(cart)
    console.log(Object.keys(cart).length == 0 + " " + Object.keys(cart).length)
    ...
    return (
        <div className="flex md:flex-row items-center bg-black text-white p-4 w-full md:justify-between">
            ...
            >
                ...
                <ul>
                    {cart && Object.keys(cart).length == 0 && <div className="text-white text-xl">No items in the cart </div>}
                    {cart && Object.keys(cart).map((k) => {
                        return (<li className="p-2 border-b flex justify-between" key={cart}>
                            <div>Shirts</div>
                            <div className="flex">
                ...
                <button className="btn bg-black mt-4 p-2 text-xl font-semibold mx-2 rounded-2xl hover:bg-gray-700 " onClick={clearCart}>Clear</button>
            </div>
        </div>
    );
};
